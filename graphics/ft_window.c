...
/*   Updated: 2022/08/05 20:33:13 by vguttenb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_window(t_general *g)
{
	...
	g->win = mlx_new_window(g->mlx, g->window_width, g->window_height * 2, "cube3D");
	if (!g->win)
		ft_error("Error de ventana", g);
	mlx_hook(g->win, 17, 1L << 17, ft_closeredcros, g);
	g->img_pov.img = mlx_new_image(g->mlx, g->window_width, g->window_height);
